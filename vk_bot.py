...
class VKBot:
    ...
    def handle_user_input(self, event):
        ...
        if msg == 'сбросить':
            ...
            self.write_msg(user_id, "Хорошо, начнём сначала. Нажмите 'Начать'.", keyboard)
            return
        ...
        if state == 'start':
            ...
            if msg in ['мужской', 'женский']:
                ...
                self.write_msg(user_id, "Укажите возрастной диапазон (например, 18-25):",
                                  self.create_keyboard_reset()
                )
            ...
            try:
                ...
                if age_from <= age_to and age_to <= 100: # Нижний порог возраста обрабатывается перехватом ошибки
    ...
    def show_candidate(self, user_id):
        ...
        candidates = get_data_persons(
            user_data['city'], user_data['age_from'], user_data['age_to'], user_data['sex']
        )
        if candidates:
            candidate_id = candidates[2]
            check_candidates = check_blacklist_db(candidate_id)
            if check_candidates is True:
                self.show_candidate(user_id)
            ...
            photo_urls = ','.join([f"photo{candidate_id}_{photo_id}" for photo_id in photo_id])
            ...
            self.write_msg(user_id, "Кандидаты не найдены. Попробуйте изменить критерии поиска.")

    def show_favourites(self, user_id):
        favourites = session.query(Favourites).filter_by(vk_user_id=user_id, chosen=True).all()
        if favourites:
